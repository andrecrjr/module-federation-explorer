...
export function resolveConfigExpressionToObject(
  ...
  if (identifier) {
    ...
    for (const statement of getProgramBody(ast)) {
      ...
      for (const declaration of Array.isArray(statement.declarations)
        ? statement.declarations.map(asNode).filter((item): item is AstNode => !!item)
        : []) {
        if (getIdentifierName(declaration.id) === identifier)
          return resolveConfigExpressionToObject(declaration.init, ast, seen);
      }
    }
  }
  ...
  if (node.type === 'ConditionalExpression') {
    return (
      resolveConfigExpressionToObject(node.consequent, ast, seen) ||
      resolveConfigExpressionToObject(node.alternate, ast, seen)
    );
  }
  if (node.type === 'LogicalExpression') {
    return (
      resolveConfigExpressionToObject(node.right, ast, seen) || resolveConfigExpressionToObject(node.left, ast, seen)
    );
  }
