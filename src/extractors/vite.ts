...
export function extractConfigFromVite(ast: AstNode, _workspaceRoot: string) {
  ...
  const callees = getImportedCallees(
    ast,
    new Set(['federation']),
    new Set(['@module-federation/vite', '@originjs/vite-plugin-federation'])
  );
  for (const plugin of nodeList(plugins.elements)) {
    ...
    if (
      pluginNode?.type === 'CallExpression' &&
      name &&
      (callees.has(name) || name.toLowerCase().includes('federation')) &&
      asNode(nodeList(pluginNode.arguments)[0])?.type === 'ObjectExpression'
    ) {
...
function findConfigObject(ast: AstNode): AstNode | undefined {
  const statement = getProgramBody(ast).find(
    item => item.type === 'ExportDefaultDeclaration' || item.type === 'ExpressionStatement'
  );
