...
export class ConfigParseError extends Error {
  constructor(public readonly diagnostics: readonly ParseDiagnostic[], cause?: unknown) {
...
function diagnosticFromError(filePath: string, error: unknown): ParseDiagnostic {
  const candidate = error as { message?: unknown; lineNumber?: unknown; column?: unknown; loc?: { line?: unknown; column?: unknown } };
  const message = typeof candidate.message === 'string' ? candidate.message : String(error);
  const line = typeof candidate.lineNumber === 'number'
    ? candidate.lineNumber
    : typeof candidate.loc?.line === 'number' ? candidate.loc.line : undefined;
  const column = typeof candidate.column === 'number'
    ? candidate.column
    : typeof candidate.loc?.column === 'number' ? candidate.loc.column : undefined;
  return { filePath, message, severity: 'error', line, column };
}

export async function parseConfigText<T>(
  content: string,
  filePath: string,
