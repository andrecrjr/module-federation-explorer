...
suite('UnifiedModuleFederationProvider', () => {
  ...
  test('only drags roots and accepts drops onto roots', async () => {
    ...
    const root: RootFolder = {
      type: 'rootFolder', path: '/workspace/host', name: 'host', configs: []
    };
    const otherRoot: RootFolder = {
      type: 'rootFolder', path: '/workspace/other', name: 'other', configs: []
    };
    let dragged: unknown;
    const transfer = {
      set: (_mime: string, item: DataTransferItem) => { dragged = item.value; }
    ...
    const dropProvider = new UnifiedModuleFederationProvider(new ExplorerStore(), {
      isRemoteRunning: () => false,
      log: () => {},
      reorderRoots: async (source, target) => { reordered.push({ dragged: source, target }); }
    });
    const dropTransfer = {
      get: () => ({ value: root })
    } as unknown as DataTransfer;
