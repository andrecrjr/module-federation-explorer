...
suite('Tree model', () => {
  test('builds remotes and exposes folders from all root configurations', () => {
    const rootFolder: RootFolder = {
      ...
      name: 'host',
      configs: [
        config('host', ['auth'], ['Shell']),
        config('host-vite', ['catalog'], ['Header'])
      ]
    };

    const children = getRootFolderChildren(rootFolder);

    assert.deepStrictEqual(children.map(child => child.type), ['remotesFolder', 'exposesFolder']);
    assert.deepStrictEqual(
      children[0].type === 'remotesFolder' ? children[0].remotes.map(remote => remote.name) : [],
      ['auth', 'catalog']
    );
    assert.deepStrictEqual(
      children[1].type === 'exposesFolder' ? children[1].exposes.map(expose => expose.name) : [],
      ['Shell', 'Header']
    );
  });
