...
suite('Package manager detection', () => {
  test('uses the project lock file and config type to build the start command', async () => {
    const result = await detectPackageManagerAndStartCommand('/workspace/remote', 'vite', async filePath =>
  ...
  test('falls back to npm start when no lock file is available', async () => {
    const result = await detectPackageManagerAndStartCommand('/workspace/host', 'webpack', async () => false);
