...
class TestDialogs implements DialogService {
  ...
  async showQuickPick<T extends QuickPickItem>(
    ...
    return (this.quickPickResults.length > 0 ? this.quickPickResults.shift() : this.quickPickResult) as
      | T
      | T[]
      | undefined;
  }
  ...
  async withProgress<T>(
    _title: string,
    task: (progress: import('../../../../app/ports').ProgressReporter) => Promise<T>
  ): Promise<T> {
    return task({ report: () => {} });
  }
}
...
suite('RootConfigManager', () => {
  ...
  test('creates a default configuration without a workspace and rejects unknown workspace choices', async () => {
    ...
    const { manager: multiManager } = createManager({
      dialogs: multiDialogs,
      folders: [
        { name: 'one', path: '/one' },
        { name: 'two', path: '/two' }
      ]
