...
function createController(
  ...
  const dependencies: RootAppControllerDependencies = {
    ...
    detectPackageManager:
      overrides.detectPackageManager || (async () => ({ packageManager: 'npm', startCommand: 'npm start' })),
...
suite('RootAppController', () => {
  ...
  test('changes a root folder only after the selected project is accepted', async () => {
    ...
    assert.equal(
      harness.rootConfigManager.savedConfigs[0].rootConfigs?.['/workspace/new-host']?.startCommand,
      'npm start'
    );
