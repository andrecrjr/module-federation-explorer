...
suite('RemoteWorkflow', () => {
  test('adds an external remote to the host selected by its path', async () => {
    ...
    assert.equal(
      harness.store.config?.rootConfigs?.['/workspace/host']?.externalRemotes?.catalog?.url,
  ...
  test('edits a remote folder, detects its package manager, and persists the change', async () => {
    ...
    assert.strictEqual(
      harness.store.config?.rootConfigs?.['/workspace/host']?.remotes?.auth?.folder,
      '/workspace/host/auth-new'
    );
  ...
  test('reports invalid external-remote prompts and missing host ownership', async () => {
    ...
    assert.strictEqual(
      harness.dialogs.inputOptions[0].validateInput?.('bad name!'),
  ...
  test('falls back to host-name lookup and supports adding to an explicit host', async () => {
    ...
    assert.strictEqual(
      hostConfig.remotes.some(remote => remote.name === 'payments'),
      true
    );
