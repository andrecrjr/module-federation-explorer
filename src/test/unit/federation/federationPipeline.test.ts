...
suite('Federation discovery pipeline', () => {
  ...
  test('parses webpack federation without package-manager or UI side effects', async () => {
    ...
    assert.deepStrictEqual(
      config.shared.map(dependency => dependency.name),
      ['react']
    );
  });
  ...
  test('extracts federation options from Vite, Modern.js, RSBuild, and Rspack shapes', async () => {
    const cases = [
      [
        `export default defineConfig({ plugins: [federation({ name: 'vite-host' })] })`,
        extractConfigFromVite,
        'vite-host'
      ],
      [
        `export default createModuleFederationConfig({ name: 'modern-host' })`,
        extractConfigFromModernJS,
        'modern-host'
      ],
      [
        `export default { moduleFederation: { options: { name: 'rsbuild-host' } } }`,
        extractConfigFromRSBuild,
        'rsbuild-host'
      ],
      [
        `new ModuleFederationPlugin({ name: 'rspack-host' })`,
        (ast: Parameters<typeof extractConfigFromWebpack>[0], root: string) =>
          extractConfigFromWebpack(ast, root, 'rspack'),
        'rspack-host'
      ]
    ] as const;
  ...
  test('preserves dynamic remote values and shared dependency options', async () => {
    ...
    assert.deepStrictEqual(
      config.remotes.map(remote => remote.name),
      ['catalog', 'dynamic', 'templated', 'configured']
    );
