...
suite('Parser expression resolution', () => {
  test('normalizes literals, identifiers, members, templates, and calls', () => {
    ...
    assert.equal(resolveStringExpression(node({
  ...
  test('resolves configuration objects through aliases and wrappers', () => {
    ...
    assert.strictEqual(resolveConfigExpressionToObject(node({ type: 'TSAsExpression', expression: identifier('config') }), ast), direct);
    assert.strictEqual(resolveConfigExpressionToObject(node({ type: 'TSTypeAssertion', expression: identifier('config') }), ast), direct);
    assert.strictEqual(resolveConfigExpressionToObject(node({ type: 'TSNonNullExpression', expression: identifier('config') }), ast), direct);
  ...
  test('resolves calls, functions, conditionals, logical expressions, and sequences', () => {
    ...
    assert.strictEqual(resolveConfigExpressionToObject(node({
      type: 'CallExpression',
      arguments: [direct]
    }), ast), direct);
    assert.strictEqual(resolveConfigExpressionToObject(node({
    ...
    assert.equal(resolveConfigExpressionToObject(node({
      type: 'ArrowFunctionExpression',
