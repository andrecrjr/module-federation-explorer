...
suite('ConfigurationService', () => {
  test('discovers all supported config types, including Rspack', async () => {
    ...
    const service = new ConfigurationService({
      findFiles: async (_rootPath, pattern) => {
        const type = pattern.includes('rspack') ? 'rspack' :
          pattern.includes('webpack') ? 'webpack' :
          pattern.includes('vite') ? 'vite' :
          pattern.includes('module-federation') ? 'modernjs' : 'rsbuild';
  ...
  test('keeps valid configs when one config file cannot be parsed', async () => {
    const service = new ConfigurationService({
      findFiles: async (_rootPath, pattern) => pattern.includes('vite')
        ? ['/workspace/valid.config.ts', '/workspace/broken.config.ts']
        : [],
