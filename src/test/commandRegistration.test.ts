...
import type { Remote, RemotesFolder, RootFolder } from '../types';

function captureHandlers(registerCommands: (register: CommandRegistrar) => vscode.Disposable[]): Map<string, CommandHandler> {
...
function createApplicationSpy(): {
  ...
  const application = {
    addRoot: async () => { calls.push({ method: 'addRoot', value: undefined }); },
    removeRoot: async (value: RootFolder) => { calls.push({ method: 'removeRoot', value }); },
    changeConfigFile: async () => { calls.push({ method: 'changeConfigFile', value: undefined }); },
    startRootApp: async (value: RootFolder) => { calls.push({ method: 'startRootApp', value }); },
    stopRootApp: async (value: RootFolder) => { calls.push({ method: 'stopRootApp', value }); },
    configureRootAppStartCommand: async (value: RootFolder) => { calls.push({ method: 'configureRootAppStartCommand', value }); },
    editRootAppCommands: async (value: RootFolder) => { calls.push({ method: 'editRootAppCommands', value }); },
    startRemote: async (value: Remote) => { calls.push({ method: 'startRemote', value }); },
    stopRemote: async (value: Remote) => { calls.push({ method: 'stopRemote', value }); },
    editRemoteCommands: async (value: Remote) => { calls.push({ method: 'editRemoteCommands', value }); },
    addExternalRemote: async (value: RemotesFolder) => { calls.push({ method: 'addExternalRemote', value }); },
    removeExternalRemote: async (value: Remote) => { calls.push({ method: 'removeExternalRemote', value }); },
    showDependencyGraph: async () => { calls.push({ method: 'showDependencyGraph', value: undefined }); },
    cleanupDisposedTerminals: () => { calls.push({ method: 'cleanupDisposedTerminals', value: undefined }); },
    reloadConfigurations: async () => { calls.push({ method: 'reloadConfigurations', value: undefined }); }
...
suite('Command registration', () => {
  test('guards root commands with the root-folder type predicate', async () => {
    ...
    assert.deepEqual(harness.calls.map(call => call.method), ['startRootApp']);
  });
  ...
  test('guards remote and remotes-folder commands before delegating', async () => {
    ...
    assert.deepEqual(harness.calls.map(call => call.method), ['startRemote', 'addExternalRemote']);
  });
  ...
  test('registers graph actions against the application boundary', async () => {
    ...
    assert.deepEqual(harness.calls.map(call => call.method), ['showDependencyGraph', 'cleanupDisposedTerminals']);
  });
});
