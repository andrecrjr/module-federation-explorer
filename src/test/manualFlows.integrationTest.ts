...
suite('Manual flow integration', () => {
  test('activates fixture workspace, loads tree, registers commands, and reloads watcher changes', async () => {
    ...
    try {
      ...
      await fs.writeFile(rootConfigPath, JSON.stringify({ roots: [path.join(workspacePath, 'host')] }, null, 2), 'utf8');
      ...
      await fs.writeFile(webpackConfigPath, originalWebpackConfig.replace('fixture-host', 'fixture-host-updated'), 'utf8');
      await waitFor(() => composition.application.getStore().getSnapshot().rootFolders[0]?.configs[0]?.name === 'fixture-host-updated');
    } finally {
