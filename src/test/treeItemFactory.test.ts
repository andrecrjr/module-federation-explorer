...
import type { ModuleFederationConfig, RootFolder } from '../types';
...
suite('TreeItemFactory', () => {
  ...
  test('uses distinct root contexts for plain, configurable, and running roots', () => {
    const roots: RootFolder[] = [
      ...
      { type: 'rootFolder', path: '/workspace/configured', name: 'configured', configs: [config()], startCommand: 'npm start' },
      { type: 'rootFolder', path: '/workspace/running', name: 'running', configs: [config()], startCommand: 'npm start', isRunning: true }
    ];
  ...
  test('renders external, running, configured, and unconfigured remotes', () => {
    ...
    const running = createTreeItem({
      name: 'auth', folder: '/workspace/auth', packageManager: 'npm', configType: 'webpack', startCommand: 'npm start'
    }, key => key === 'remote-auth');
    const configured = createTreeItem({
      name: 'catalog', folder: '/workspace/catalog', packageManager: 'pnpm', configType: 'vite', startCommand: 'pnpm dev', buildCommand: 'pnpm build'
    }, () => false);
    const unconfigured = createTreeItem({
      name: 'payments', folder: '', packageManager: 'npm', configType: 'webpack'
    }, () => false);
