...
import type { ModuleFederationConfig, Remote, RootFolder, UnifiedRootConfig } from '../types';
...
class MemoryRootConfig implements RootConfigService {
  ...
  async hasConfiguredRoots(): Promise<boolean> { return this.configured; }
  async loadRootConfig(): Promise<UnifiedRootConfig | null> { return this.config; }
  getConfigPath(): string | undefined { return '/workspace/.vscode/mf-explorer.roots.json'; }
  async setConfigPath(_configPath: string): Promise<void> {}
  async saveRootConfig(config: UnifiedRootConfig): Promise<void> { this.config = config; }
  async addRoot(rootPath: string): Promise<void> { this.config?.roots.push(rootPath); }
  async removeRoot(rootPath: string): Promise<void> { this.config = this.config ? { ...this.config, roots: this.config.roots.filter(item => item !== rootPath) } : null; }
  async changeConfigFile(): Promise<boolean> { return true; }
}
...
class TestDialogs implements DialogService {
  ...
  async showError(message: string, _options?: DialogMessageOptions): Promise<string | undefined> { this.errors.push(message); return undefined; }
  async showSuccess(message: string): Promise<void> { this.successes.push(message); }
  async showInput(): Promise<string | undefined> { return this.inputResult; }
  ...
  async showConfirmation(): Promise<boolean> { return this.confirmationResult; }
...
class RecordingTerminalManager implements TerminalPort {
  ...
  getRunningRemoteTerminal(_remoteKey: string): TerminalLike | undefined { return this.runningRemoteTerminal; }
  stopRemote(remoteKey: string): void { this.stoppedRemotes.push(remoteKey); }
  setRunningRootApp(_rootPath: string, _terminal: TerminalLike): void {}
  isRootAppRunning(_rootPath: string): boolean { return this.rootRunning; }
  stopRootApp(rootPath: string): boolean { this.stoppedRoots.push(rootPath); return true; }
  clearAllRunningApps(): void { this.cleared++; }
  cleanupDisposedTerminals(): { remotes: number; rootApps: number } { return this.cleanupResult; }
  handleTerminalClosed(_closedTerminal: TerminalLike): boolean { return this.terminalClosed; }
}
...
function createServices(options: {
  ...
  const dependencyGraphManager: DependencyGraphService = {
    refreshDependencyGraph: () => { graphCalls.refresh++; },
    generateDependencyGraph: () => { graphCalls.generate++; return graph; },
    showDependencyGraph: () => { graphCalls.show++; }
  };
  const host: ApplicationHostPort = {
    executeCommand: async command => { executedCommands.push(command); },
    setContext: async (key, value) => { contextValues.set(key, value); },
    withProgress: async <T>(_title: string, task: (progress: ProgressReporter) => Promise<T>) => task({ report: () => {} }),
    showErrorMessage: async message => { hostErrors.push(message); },
  ...
  return {
    services: {
      ...
      path: pathPort,
      host,
      trackSuccess: async event => { trackedEvents.push(event); }
    },
    rootConfig,
    dialogs,
    terminalManager,
...
suite('ExplorerApplication', () => {
  ...
  test('shows an informational message instead of opening an empty graph', async () => {
    ...
    assert.deepEqual(harness.dialogs.infos, ['No Module Federation configurations found. Please add a Host folder first.']);
  ...
  test('reports configuration load errors through both logging boundaries', async () => {
    ...
    assert.deepStrictEqual(harness.hostErrors, ['Failed to load Module Federation configurations. See output panel for details.']);
  });
  ...
  test('starts an unconfigured remote after folder and command setup', async () => {
    ...
    assert.deepStrictEqual(harness.terminalManager.startedRemotes, [{
      key: 'remote-catalog', name: 'catalog', folder: '/workspace/host/catalog', build: 'npm run build', start: 'npm run start'
    }]);
