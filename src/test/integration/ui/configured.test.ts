...
suite('Desktop UI smoke tests', function (this: Mocha.Suite) {
  ...
  test('edits and persists a remote build command through the tree action', async () => {
    ...
    await waitFor(
      async () =>
        (await readFixtureConfig()).rootConfigs?.[state.rootPath]?.remotes?.auth?.buildCommand === updatedCommand,
      15000,
  ...
  test('keeps a host when its removal is canceled', async () => {
    ...
    const confirmationMessage = (await confirmation.getDetails()).replace(/\s+/g, ' ');
    assert.ok(
      confirmationMessage.includes(`Are you sure you want to remove "${state.rootPath}" from the configuration?`)
    );
  ...
  test('opens the graph webview and handles a node click', async () => {
    ...
    await waitFor(async () =>
      (await new Workbench().getEditorView().getOpenEditorTitles()).includes('Module Federation Explorer Graph')
    );
    ...
    try {
      ...
      await waitFor(
        ...
        15000,
        'Graph host node did not render'
      );
      ...
      await waitFor(
        ...
        15000,
        'Graph host node did not settle inside the webview'
      );
...
async function findGraphNode(graph: WebView, ariaLabel: string): Promise<WebElement> {
  ...
  await waitFor(
    ...
    15000,
    `Graph node did not render: ${ariaLabel}`
  );
