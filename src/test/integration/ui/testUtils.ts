...
export async function findTreeItem(label: string, maxLevel = 0, timeoutMs = 15000): Promise<CustomTreeItem> {
  ...
  try {
    await waitFor(
      ...
      timeoutMs,
      `Tree item not found: ${label}`
    );
  } catch (error) {
    const visible =
      visibleItems.length > 0 ? ` Visible items: ${visibleItems.join(', ')}.` : ' No visible tree rows found.';
...
export async function selectTreeContextAction(label: string, action: string, maxLevel = 0): Promise<void> {
  await waitFor(
    async () => {
    ...
    15000,
    `Could not select '${action}' for tree item '${label}'`
  );
}

export async function clickTreeItemAction(label: string, action: string, maxLevel = 0): Promise<void> {
  await waitFor(
    async () => {
      const item = await findTreeItem(label, maxLevel);
    ...
    15000,
    `Could not click '${action}' for tree item '${label}'`
  );
}
