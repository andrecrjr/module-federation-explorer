...
suite('Desktop onboarding UI smoke test', function (this: Mocha.Suite) {
  ...
  suiteSetup(async function (this: Mocha.Context) {
    this.timeout(120000);
    await openWorkspaceResource(getFixtureWorkspacePath('ui-onboarding'));
  });
  ...
  test('configures the detected host from the onboarding webview', async () => {
    ...
    await waitFor(
      async () => (await new EditorView().getOpenEditorTitles()).includes('Module Federation Setup'),
      15000
    );
    ...
    assert.equal(
      (await new Workbench().getEditorView().getOpenEditorTitles()).includes('Module Federation Setup'),
      false
    );
  });
});
