...
function isRemoteConfig(value: unknown): boolean {
  ...
  return typeof value.name === 'string' &&
    ...
    optionalString(value, 'url') && optionalString(value, 'remoteEntry') &&
    optionalString(value, 'startCommand') && optionalString(value, 'buildCommand') &&
...
function isExternalRemoteConfig(value: unknown): boolean {
  return isJsonRecord(value) && typeof value.name === 'string' && typeof value.url === 'string' &&
    value.configType === 'external' && value.isExternal === true;
}

function isRootConfigEntry(value: unknown): boolean {
  if (!isJsonRecord(value) || !optionalString(value, 'startCommand')) return false;
  if (value.remotes !== undefined && (!isJsonRecord(value.remotes) || !Object.values(value.remotes).every(isRemoteConfig))) return false;
  if (value.externalRemotes !== undefined && (!isJsonRecord(value.externalRemotes) || !Object.values(value.externalRemotes).every(isExternalRemoteConfig))) return false;
  return true;
}
...
export function parseRootConfig(value: unknown): UnifiedRootConfig | undefined {
  if (!isJsonRecord(value) || !Array.isArray(value.roots) || !value.roots.every(root => typeof root === 'string')) return undefined;
